...
def connect():
    ...
    s.settimeout(60)
    s.connect(('192.168.2.xx', 22))
    t = paramiko.Transport(s)
    t.connect(username='kai', password='********')
...
sys.path.insert(0, "~/lumilearn/deploy_model")
from inference import LumiLearnInference

app = Flask(__name__)

MODEL_DIR = "~/lumilearn/deploy_model"
MODEL_VERSION = "v5"
DEVICE = "cpu"
...
cmd = f"""cat > ~/lumilearn/inference_server_tianhong.py << 'PYEOF'
{server_code}
PYEOF
echo "SERVER_WRITTEN"
"""
...
out, err = sh(
    'grep -c "weights_only=False" ~/lumilearn/deploy_model/inference.py 2>&1 || echo "NEED_FIX"',
    timeout=10
)
...
python3 -c "
import re
with open('~/lumilearn/deploy_model/inference.py', 'r') as f:
    c = f.read()
...
c = c.replace(old, new)
with open('~/lumilearn/deploy_model/inference.py', 'w') as f:
    f.write(c)
...
launch = (
    ...
    '> ~/lumilearn/inference_server.log 2>&1 &\n'
    'disown\n'
    'echo LAUNCHED\n'
)
channel.send(launch)
...
if '"status":"ok"' not in out:
    print("\nServer log:")
    out, _ = sh('tail -20 ~/lumilearn/inference_server.log 2>&1', timeout=10)
    print(out[:2000])
    print("\nProcesses:")
...
if '"response"' in out:
    ...
    print(f"  URL: http://192.168.2.xx:18080")
