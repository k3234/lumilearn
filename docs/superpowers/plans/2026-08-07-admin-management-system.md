...
class TestAdminAPI(unittest.TestCase):
    ...
    def _login(self):
        resp = self.client.post("/api/admin/login", json={"username": "admin", "password": "[REDACTED]"})
...
<div class="login-wrap" id="loginWrap">
  <div class="login-card">
    ...
    <input type="password" id="password" placeholder="密码" value="[REDACTED]">
...
Expected: 访问 `http://localhost:18080/admin` 显示登录页；用 `[REDACTED_CREDS]` 登录后看到管理面板
...
包含：
- 默认账号与首次登录安全建议（`admin` / `[REDACTED]`，务必修改密码）
