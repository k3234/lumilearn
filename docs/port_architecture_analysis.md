...
# 或指定内网端口
python -m framework.api.server --port 18080 --host 10.0.1.xx
```

**访问方式**:
- 学生访问: `http://10.0.1.xx/` 或 `http://lumilearn.example.org/`
- 教师访问: `http://10.0.1.xx/learn`
- API 调用: `http://10.0.1.xx/api/...` (自动跟随)
...
server {
    listen 80;
    server_name lumilearn.example.org;
