...
| 2026-08-20 | **仓库整理与隐私脱敏**：`git rm --cached` 根目录 6 个临时调试脚本 + `output/*.json` + `push_git.ps1`（本地 token 工具）；移除 7 个含远程主机用户名的历史 `_deploy_*`/`_check_remote*` 脚本；`.gitignore` 增加对应规则与 `output/`、`test_migration.py`；`docs/CHANGELOG_*.md` 中远程主机家目录脱敏为 `/home/<user>/` |
