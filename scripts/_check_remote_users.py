...
print("=== 检查远程服务器用户 ===")
try:
    r = requests.get("http://192.168.2.68:18080/api/admin/users",
...
# 先登录 admin
print("\n=== Admin 登录 ===")
r = requests.post("http://192.168.2.68:18080/api/admin/login",
        ...
        for pw in ["test1234", "123456", "password", "student", "demo", "12345678"]:
            ...
            if d.get("code") == 0:
                ...
                if d2.get("code") == 0:
                    ...
                    # 测试 step
                    r3 = requests.post("http://192.168.2.68:5010/api/learn/step",
...
for u in users:
    if u.get("role") == "teacher":
        ...
        for pw in ["test1234", "123456", "password", "teacher", "12345678"]:
            r = requests.post("http://192.168.2.68:5001/api/login",
                              json={"username": uname, "password": pw}, timeout=5)
            d = r.json()
            if d.get("success"):
...
for u in users:
    if u.get("role") == "student":
        ...
        for pw in ["test1234", "123456", "password", "student", "demo"]:
            r = requests.post("http://192.168.2.68:5000/api/auth/login",
                              json={"username": uname, "password": pw}, timeout=5)
            d = r.json()
            if d.get("code") == 0:
                print("  GOAI 学生登录成功: username=%s password=%s" % (uname, pw))
                # 测试学习
                r2 = requests.post("http://192.168.2.68:5000/api/learn/start",
                ...
                if d2.get("code") == 0:
                    sid_str = d2["data"].get("id", "")
                    r3 = requests.post("http://192.168.2.68:5000/api/learn/step",
                                       json={"sessionId": sid_str, "step": 1}, timeout=30)
                    d3 = r3.json()
                    print("  step1: code=%d has_content=%s" % (d3.get("code"), bool((d3.get("data") or {}).get("content"))))
                break
...
print("\n=== 前端页面检查 ===")
r = requests.get("http://192.168.2.68:5010/index.html", timeout=5)
print("5010/index.html: HTTP %d has_real=%s" % (r.status_code, "__LUMILEARN_REAL__" in r.text))
r = requests.get("http://192.168.2.68:5000/proto/index.html", timeout=5)
