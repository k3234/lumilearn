...
cmd = f"""cat > ~/lumilearn/smart_reply_engine.py << 'PYEOF'
{engine_code}
PYEOF
echo "UPLOADED"
"""
...
sf.putfo(io.BytesIO(test_script.encode()), '~/lumilearn/test_smart_reply_tianhong.py')
sf.close()

stdin, stdout, stderr = ssh.exec_command(
    '/usr/bin/python3 ~/lumilearn/test_smart_reply_tianhong.py 2>&1',
    timeout=120
)
out = stdout.read().decode(errors='replace')
err = stderr.read().decode(errors='replace')
...
if "DEPLOY_SUCCESS" in out:
    ...
    print()
    print("  推理服务器: http://192.168.2.xx:18080")
    print("  智能引擎:   ~/lumilearn/smart_reply_engine.py")
    print()
    print("  本地启动直播助手:")
